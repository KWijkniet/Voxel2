...
public static class MeshBuilder
{
                                                  ...
                                                  int lodLevel = 0)
    {
        ...
        GreedyMeshFace(voxels, neighbours[0], 0, 1, 2, Vector3.right,   false, _vertsTS, _normsTS, _uvsTS, _trisTS, _maskTS, step);
        GreedyMeshFace(voxels, neighbours[1], 0, 1, 2, Vector3.left,    true,  _vertsTS, _normsTS, _uvsTS, _trisTS, _maskTS, step);
        GreedyMeshFace(voxels, neighbours[2], 1, 2, 0, Vector3.up,      false, _vertsTS, _normsTS, _uvsTS, _trisTS, _maskTS, step);
        GreedyMeshFace(voxels, neighbours[3], 1, 2, 0, Vector3.down,    true,  _vertsTS, _normsTS, _uvsTS, _trisTS, _maskTS, step);
        GreedyMeshFace(voxels, neighbours[4], 2, 0, 1, Vector3.forward, false, _vertsTS, _normsTS, _uvsTS, _trisTS, _maskTS, step);
        GreedyMeshFace(voxels, neighbours[5], 2, 0, 1, Vector3.back,    true,  _vertsTS, _normsTS, _uvsTS, _trisTS, _maskTS, step);
        ...
        byte[] mask, int step)
    {
        ...
        for (int slice = 0; slice < cells; slice++)
        {
            ...
            for (int u = 0; u < cells; u++)
            {
                ...
                byte here = voxels[pos0 + pos1 * size + pos2 * size * size];
        ...
        byte[] mask, int step)
    {
        ...
        for (int slice = 0; slice < cellsSlice; slice++)
        {
            ...
            for (int u = 0; u < cellsU; u++)
            {
                ...
                byte here = region.GetBlockUnchecked(pos0, pos1, pos2);
...
public sealed class WritableMeshData
{
    ...
    public Mesh Apply()
    {
        ...
        { positions[i] = _vertices[i]; normals[i] = _normals[i]; uvCoords[i] = _uvs[i]; }
