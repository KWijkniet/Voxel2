using System.Collections.Generic;
using UnityEngine;

public static class GreedyMeshGeneratorBackup
{
    private static byte[] voxels;
    private static int size;
    private static List<Vector3> vertices = new List<Vector3>();
    private static List<int> triangles = new List<int>();
    private static List<Vector2> uvs = new List<Vector2>();

    // Directions for neighbor checking
    private static Vector3Int[] faceOffsets =
    {
        new Vector3Int( 0,  0,  1), // Front
        new Vector3Int( 0,  0, -1), // Back
        new Vector3Int( 0,  1,  0), // Top
        new Vector3Int( 0, -1,  0), // Bottom
        new Vector3Int( 1,  0,  0), // Right
        new Vector3Int(-1,  0,  0)  // Left
    };

    // Face vertices relative to voxel position
    private static Vector3[,] faceQuads =
    {
        { new Vector3(0, 0, 1), new Vector3(1, 0, 1), new Vector3(0, 1, 1), new Vector3(1, 1, 1) }, // Front
        { new Vector3(0, 0, 0), new Vector3(1, 0, 0), new Vector3(0, 1, 0), new Vector3(1, 1, 0) }, // Back
        { new Vector3(0, 1, 1), new Vector3(1, 1, 1), new Vector3(0, 1, 0), new Vector3(1, 1, 0) }, // Top
        { new Vector3(0, 0, 0), new Vector3(1, 0, 0), new Vector3(0, 0, 1), new Vector3(1, 0, 1) }, // Bottom
        { new Vector3(1, 0, 1), new Vector3(1, 0, 0), new Vector3(1, 1, 1), new Vector3(1, 1, 0) }, // Right
        { new Vector3(0, 0, 0), new Vector3(0, 0, 1), new Vector3(0, 1, 0), new Vector3(0, 1, 1) }  // Left
    };

    // Corrected triangle order for outward-facing faces
    private static int[,] faceTriangles =
    {
        { 0, 1, 2, 1, 3, 2 }, // Back
        { 0, 2, 1, 1, 2, 3 }, // Front
        { 0, 1, 2, 1, 3, 2 }, // Top
        { 0, 1, 2, 1, 3, 2 }, // Bottom
        { 0, 1, 2, 2, 1, 3 }, // Right
        { 0, 1, 2, 1, 3, 2 }  // Left
    };

    public static Mesh GenerateGreedyMesh(byte[] _voxels, int _size) {
        size = _size;
        voxels = _voxels;
        vertices.Clear();
        triangles.Clear();
        uvs.Clear();

        for (int i = 0; i < voxels.Length; i++) {
            Vector3Int pos = Helpers.IndexToCoordinates(i, size);
            if (voxels[i] == 1) {
                AddVoxel(pos.x, pos.y, pos.z);
            }
        }

        // ApplyGreedy();

        Mesh mesh = new Mesh();
        mesh.vertices = vertices.ToArray();
        mesh.triangles = triangles.ToArray();
        mesh.uv = uvs.ToArray();
        mesh.RecalculateNormals();

        return mesh;
    }

    private static void AddVoxel(int x, int y, int z) {
        for (int i = 0; i < 6; i++) {
            int nx = x + faceOffsets[i].x;
            int ny = y + faceOffsets[i].y;
            int nz = z + faceOffsets[i].z;

            // If neighbor is air, draw face
            if (GetVoxel(nx, ny, nz, size) == 0) {
                int vertexIndex = vertices.Count;

                for (int j = 0; j < 4; j++) {
                    vertices.Add(new Vector3(x, y, z) + faceQuads[i, j]);
                    uvs.Add(new Vector2(j % 2, j / 2));
                }

                for (int j = 0; j < 6; j++) {
                    triangles.Add(vertexIndex + faceTriangles[i, j]);
                }
            }
        }
    }

    private static byte GetVoxel(int x, int y, int z, int size) {
        if (x < 0 || x >= size || y < 0 || y >= size || z < 0 || z >= size)
            return 0; // Treat out-of-bounds as empty space
        return voxels[Helpers.CoordinatesToIndex(x, y, z, size)];
    }

    private static void ApplyGreedy() {
        List<Vector3> newVertices = new List<Vector3>();
        List<int> newTriangles = new List<int>();
        List<Vector2> newUvs = new List<Vector2>();

        bool[,,] visited = new bool[size, size, size];

        for (int d = 0; d < 3; d++) // 0=X, 1=Y, 2=Z
        {
            int u = (d + 1) % 3;
            int v = (d + 2) % 3;

            Vector3Int[] axisDirs = { Vector3Int.right, Vector3Int.up, Vector3Int.forward };

            int[] dims = { size, size, size };
            int dimU = dims[u], dimV = dims[v], dimD = dims[d];

            for (int dPos = 0; dPos < dimD; dPos++)
            {
                bool[,] mask = new bool[dimU, dimV];

                // Step 1: Generate mask of faces to be merged
                for (int uPos = 0; uPos < dimU; uPos++)
                {
                    for (int vPos = 0; vPos < dimV; vPos++)
                    {
                        int x = d == 0 ? dPos : (u == 0 ? uPos : vPos);
                        int y = d == 1 ? dPos : (u == 1 ? uPos : vPos);
                        int z = d == 2 ? dPos : (u == 2 ? uPos : vPos);

                        if (visited[x, y, z] || voxels[Helpers.CoordinatesToIndex(x, y, z, size)] == 0)
                            continue;

                        int nx = x + axisDirs[d].x;
                        int ny = y + axisDirs[d].y;
                        int nz = z + axisDirs[d].z;

                        if (GetVoxel(nx, ny, nz, size) == 0) 
                            mask[uPos, vPos] = true;
                    }
                }

                // Step 2: Apply greedy merging
                for (int uPos = 0; uPos < dimU; uPos++)
                {
                    for (int vPos = 0; vPos < dimV; vPos++)
                    {
                        if (!mask[uPos, vPos]) continue;

                        int width = 1, height = 1;

                        // Expand width
                        while (uPos + width < dimU && mask[uPos + width, vPos])
                            width++;

                        // Expand height
                        bool expand = true;
                        while (vPos + height < dimV && expand)
                        {
                            for (int i = 0; i < width; i++)
                            {
                                if (!mask[uPos + i, vPos + height])
                                {
                                    expand = false;
                                    break;
                                }
                            }
                            if (expand) height++;
                        }

                        // Mark visited
                        for (int i = 0; i < width; i++)
                            for (int j = 0; j < height; j++)
                                mask[uPos + i, vPos + j] = false;

                        // Create quad
                        Vector3 p1 = Vector3.zero, p2 = Vector3.zero, p3 = Vector3.zero, p4 = Vector3.zero;

                        p1[d] = dPos; p2[d] = dPos; p3[d] = dPos; p4[d] = dPos;
                        p1[u] = uPos; p1[v] = vPos;
                        p2[u] = uPos + width; p2[v] = vPos;
                        p3[u] = uPos; p3[v] = vPos + height;
                        p4[u] = uPos + width; p4[v] = vPos + height;

                        int vertIndex = newVertices.Count;
                        newVertices.Add(p1);
                        newVertices.Add(p2);
                        newVertices.Add(p3);
                        newVertices.Add(p4);

                        newUvs.Add(new Vector2(0, 0));
                        newUvs.Add(new Vector2(1, 0));
                        newUvs.Add(new Vector2(0, 1));
                        newUvs.Add(new Vector2(1, 1));

                        newTriangles.Add(vertIndex);
                        newTriangles.Add(vertIndex + 2);
                        newTriangles.Add(vertIndex + 1);
                        newTriangles.Add(vertIndex + 1);
                        newTriangles.Add(vertIndex + 2);
                        newTriangles.Add(vertIndex + 3);
                    }
                }
            }
        }

        vertices = newVertices;
        triangles = newTriangles;
        uvs = newUvs;
    }
}
